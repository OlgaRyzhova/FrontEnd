...
var task = document.querySelector('.main_page');
document.addEventListener('keydown', function (event) {
  ...
  if (event.code == 'KeyS' && (event.ctrlKey || event.metaKey)) {
    ...
    var _textElement = document.createElement('div');

    _textElement.setAttribute('id', 'text');

    _textElement.textContent = _textarea.value;

    _textarea.remove();

    task.appendChild(_textElement);
  }
}); // TASK 2
...
function sortTable(number) {
  ...
  tableBody.innerHTML = ''; //сортируем масссив tableRows 

  sortedRows.sort(function (a, b) {
    var aCols = a.querySelectorAll('td');
    var bCols = b.querySelectorAll('td');
    var aOne = aCols[number].textContent;
    var bOne = bCols[number].textContent;
